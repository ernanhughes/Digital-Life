...
We did not replace the frozen Digital Crystal.

The original sequential-RNG model remained unchanged.
...
Across 96 runs per implementation, the omnibus morphology comparison found no evidence of a gross distributional discrepancy, with `p ≈ 0.922`.
...
This did not prove that the two stochastic processes were mathematically identical.
...
In the exploratory run, by elapsed step 20 the sequentially coupled pulse branch had reached about 87 percent of the symmetric-difference scale of independently reseeded continuations.

Under common random numbers, it remained at only about 11 percent.
...
Immediately after the complete sequence, their average normalized symmetric difference was approximately:
...
The frozen primary angular test returned:
...
The secondary 24-feature test was even less suggestive:
...
We did not test every sequence.
...
The formal status was therefore:
...
Failed within the declared experimental scope. 
...
Different histories produced different particular futures.
...
The crystal has history.

But history is not automatically state.

It has causal consequences.

But causal consequences are not automatically recoverable information.

It can be changed.

But being changed is not the same as remembering what changed it.
...
We want digital life.

We built a primitive growing process.
...
We should not add a variable called:
...
CAUSAL CONSEQUENCE
↓
PERSISTENT INTERNAL CONSEQUENCE
↓
