...
With all of that in place, here is the synthesis. It is provisional: a new claim rather than an experimental result, and one that should be attacked immediately.
...
This is not yet life; it is a candidate foundation. That qualification is essential. The phrase *organized causal consequence* also needs anchoring in the experimental record rather than being left as an abstraction.
...
That does not automatically invalidate the proposal. A candidate foundation may be broader than the phenomenon ultimately built from it, but that breadth creates the next experimental obligation: identify what ordinary stateful processes lack.
...
The Digital Crystal has the same unresolved gap. Its local rule, execution environment and evaluation machinery were all supplied from outside. Its organization can affect subsequent events inside that machinery, but no experiment in this book established that the organization itself helps maintain, reconstruct or modify the conditions responsible for its own continuation.

We did not test that, so the capability is **unestablished**, not absent.
...
The question marks matter because only the first three stages are motivated by direct experimental results in this book, although the hidden-state result remains a causal-sufficiency result rather than evidence of endogenous history encoding. The later stages are research hypotheses exposed by what those experiments did not establish.
