...
Dead or alive if we want convenient terminology – although even those words are unnecessary. We could equally call them OFF / ON.
...
including itself. Nine bits. That means `2⁹ = 512` possible neighborhood configurations.
...
No organisms. No energy. No genome. No reproduction function. No notion of an individual. No `Life` class.
...
Self‑replication was not explicitly programmed into a candidate organism. In fact, there was no candidate organism.

The search found a **rule for the universe**. Then something appeared inside that universe.
...
The second case does not prove life. But it removes an enormous amount of cargo cult.

---

## Start Almost From Nothing

The published rule can produce interesting behavior from sparse random initial states. Under appropriate sparse conditions, small shape‑changing clusters emerge.
...
That is already strange. Replication appears at more than one scale.
...
Imagine: `A` appears, and later `A     A`.  
Did the first `A` create the second? Or did the dynamics simply happen to create another identical pattern nearby?
...
Then connect those dependencies. At the cell level:
...
Now we can construct an ancestry graph – not merely “these patterns look similar”, but “this pattern causally contributed to the existence of that pattern”.
...
flowchart TD
    ...
    subgraph Causal
    C1[Cell at t] -->|necessary for| C2[Cell at t+1]
    C2 --> C3[Aggregate to clusters]
    C3 --> C4[Parent cluster causally<br/>produces offspring cluster]
    end
    V1 -.-x "Not sufficient" 
    V2 -.-x "for ancestry"
```
...
Within the first 10,000 updates the researchers found **433** copies causally descending from that seed.
...
We were already asking whether digital life really needs one body, one place, one boundary.
...
The "thing" is in the relationships. Not necessarily in one body.
...
Absolutely not. At least, the research does not establish that.
...
For studying morphology, movement, continuous bodies, mass flow, and multispecies interactions, Flow‑Lenia may be the better substrate. We will absolutely return to it.

But Outlier has one enormous advantage for us: we can see almost all the machinery.
...
No neural network. No hidden model. No floating‑point organism representation. No complicated controller.
...
That's it. Let's build the universe.

---

## Decode the Rule

A Moore neighborhood has nine cells. We number them:
...
This convention is used by the `MAP` rule representation: every possible `3 × 3` binary neighborhood maps to an integer from `0` through `511`, and the corresponding bit in the 512‑entry table gives the next center‑cell state.
...
Now `RULE = decode_map_rule(OUTLIER_MAP)` is the entire local physics of our universe.
...
```python
def outlier_step(state: np.ndarray, rule: np.ndarray) -> np.ndarray:
    nw = np.roll(np.roll(state, 1, axis=0), 1, axis=1)
    north = np.roll(state, 1, axis=0)
    ne = np.roll(np.roll(state, 1, axis=0), -1, axis=1)
    west = np.roll(state, 1, axis=1)
    centre = state
    east = np.roll(state, -1, axis=1)
    sw = np.roll(np.roll(state, -1, axis=0), 1, axis=1)
    south = np.roll(state, -1, axis=0)
    se = np.roll(np.roll(state, -1, axis=0), -1, axis=1)
...
There is no `replicate()`, no `organism()`, no `find_child()`, no `evolve()`. Only the transition rule.
...
def make_world(size: int = 512) -> np.ndarray:
    ...
    row = size // 2 - 1
    col = size // 2 - 1
    world[row:row+3, col:col+3] = SEED
    return world
```
...
for tick in range(5000):
    world = outlier_step(world, RULE)
    if tick % 100 == 0:
        print(tick, int(world.sum()))
```
...
**Scale matters.** A phenomenon may not exist at all until the universe is large enough.
...
1. **Do not design the organism.** Design or discover the universe and let candidate entities form within it.  
2. **Allow multiple scales.** Do not assume cell → organism. Perhaps cell → cluster → formation → colony → superstructure, with life‑like properties appearing at different levels.  
3. **Do not require connected bodies.** An individual may be distributed rather than one contiguous structure.  
4. **Track causation.** Appearance is not enough. If we claim reproduction, inheritance or influence, reconstruct who actually caused whom.  
5. **Let interaction generate novelty.** Collision and recombination may be more interesting than injecting random mutation from outside.
...
Perhaps it is the **floor**. It tells us how much can happen before intelligence enters the system at all.

```mermaid
flowchart TD
    A[Outlier: binary local rules] --> B[Emergent replicators<br/>Distributed individuals]
    B --> C[Causal ancestry<br/>Branching lineages]
    C --> D[What's missing?<br/>Learning, adaptation,<br/>self-modification,<br/>information assimilation]
    D --> E[Next steps: add capabilities<br/>from the digital substrate]
```
