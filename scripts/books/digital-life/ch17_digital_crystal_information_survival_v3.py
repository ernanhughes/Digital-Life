...
# ---------------------------------------------------------------------------
# Equal-exposure eligibility
# ---------------------------------------------------------------------------

def cumulative_pulses(codeword: Sequence[int], elapsed_step: int) -> int:
    """
    Number of transmitted 1-bits delivered by elapsed_step.

    elapsed_step is 1-indexed from transmission start. Any steps beyond the
    codeword length add no further pulses because the channel is off.
    """
    upto = min(int(elapsed_step), len(codeword))
    return int(sum(int(v) for v in codeword[:upto]))


def equal_exposure_at_step(level: LadderLevel, elapsed_step: int) -> bool:
    """
    True only if every competing codeword has delivered the same cumulative
    number of pulses by this observation time.

    This is the key v3 correction. During unequal prefix exposure, decoding can
    rediscover Chapter 16's causal sensitivity (pulse happened vs did not
    happen). Those observations remain useful diagnostics but are not eligible
    evidence for temporal-information survival.
    """
    counts = [
        cumulative_pulses(codeword, elapsed_step)
        for codeword in level.codewords
    ]
    return len(set(counts)) == 1


def exposure_diagnostics(level: LadderLevel, observation_steps: Sequence[int]) -> dict:
    rows = []
    for step in observation_steps:
        counts = [
            cumulative_pulses(codeword, step)
            for codeword in level.codewords
        ]
        rows.append({
            "elapsed_step": int(step),
            "cumulative_pulses_by_codeword": counts,
            "equal_exposure": len(set(counts)) == 1,
            "eligible_for_information_survival_claim": len(set(counts)) == 1,
            "role": (
                "information_survival"
                if len(set(counts)) == 1
                else "causal_onset_diagnostic_only"
            ),
        })
    return {
        "level": level.level,
...
class Reporter:
    ...
    def full_report(self, metadata: dict) -> Path:
        path = self.root / "ch17-v3-full-experimental-report.md"
        header = (
            "# Chapter 17 — What Survives the Channel? v3 Experimental Report\n\n"
            "## Run metadata\n\n```json\n"
...
def stage_2_collect(reporter, profile, params, levels, seed):
    ...
    eligibility = {}
    for level in levels:
        steps = sorted(set(r["elapsed_step"] for r in all_rows[level.level]))
        eligibility[level.level] = exposure_diagnostics(level, steps)
    ...
    result = {
        ...
        "equal_exposure_eligibility": eligibility,
        "eligibility_rule": (
            "An observation can support temporal-information survival only when "
            "all competing codewords have delivered the same cumulative pulse "
            "count by that observation time. Unequal-prefix observations are "
            "causal-onset diagnostics only."
        ),
    }
...
def stage_4_nulls(reporter, profile, levels, all_rows, decode_results, seed):
    ...
    for level in levels:
        ...
        for elapsed in steps:
            ...
            eligible = equal_exposure_at_step(level, elapsed)
            headline[level.level][str(elapsed)] = {
                ...
                "equal_exposure": eligible,
                "eligible_for_information_survival_claim": eligible,
                "role": (
                    "information_survival"
                    if eligible
                    else "causal_onset_diagnostic_only"
...
def stage_5_boundary(reporter, profile, levels, decode_results, null_result):
    print("\n=== STAGE 5 — TEMPORAL INFORMATION SURVIVAL BOUNDARY ===")

    boundary = {}
    first_supported_level = None
    first_supported_step = None
    first_supported_after_lag = None
    causal_onset_hits = []
    ...
    for level in levels:
        ...
        for r in sorted(
            ...
            equal_exposure = equal_exposure_at_step(level, r["elapsed_step"])

            # Same exploratory numerical threshold as v2, but v3 adds the
            # essential equal-exposure eligibility condition.
            numerical_pass = (
                r["primary_accuracy"] > q95
                and margin >= 0.075
            )
            survives = numerical_pass and equal_exposure
            ...
            entry = {
                ...
                "eligible_for_information_survival_claim": equal_exposure,
                "passes_information_survival_threshold": survives,
                "role": (
                    "information_survival"
                    if equal_exposure
                    else "causal_onset_diagnostic_only"
            ...
            if numerical_pass and not equal_exposure:
                causal_onset_hits.append({
                    "level": level.level,
                    "elapsed_step": r["elapsed_step"],
                    "accuracy": r["primary_accuracy"],
                    "chance": r["chance_accuracy"],
                    "null_q95": q95,
                    "interpretation": (
                        "Decoder separated branches during unequal cumulative "
                        "pulse exposure. This is compatible with Chapter 16 causal "
                        "sensitivity and is not evidence that timing survives after "
                        "equal exposure."
                    ),
                })

            if survives and first_supported_level is None:
                first_supported_level = level.level
                first_supported_step = r["elapsed_step"]
                first_supported_after_lag = r["after_lag"]

        boundary[level.level] = entries

    result = {
        "exploratory_numerical_threshold": (
            "raw combined accuracy > grouped permutation q95 AND >= 0.075 "
            "absolute accuracy above chance"
        ),
        "eligibility_rule": (
            "A temporal-information claim additionally requires equal cumulative "
            "pulse exposure across all competing codewords at the observation time."
        ),
        "first_supported_level": first_supported_level,
        "first_supported_elapsed_step": first_supported_step,
        "first_supported_after_lag": first_supported_after_lag,
        "causal_onset_diagnostic_hits": causal_onset_hits,
        "levels": boundary,
        "interpretation": (
            "v3 explicitly separates causal-onset decoding from temporal-information "
            "survival. Unequal-prefix observations can never establish the latter."
        ),
    }
    reporter.json("stage-05-information-survival-boundary.json", result)
    reporter.stage(
        "stage-05-information-survival-boundary.md",
        "Stage 5 — What Survives After Equal Exposure?",
        f"""The v2 confound is removed here.
...
def stage_6_verdict(reporter, boundary_result):
    ...
    after_lag = boundary_result["first_supported_after_lag"]
    causal_hits = boundary_result["causal_onset_diagnostic_hits"]

    if level is None:
        if causal_hits:
            verdict = "TRANSIENT_CAUSAL_DISTINCTION_OBSERVED"
            claim = (
                "Receiver-only decoding sometimes separated codeword branches "
                "during unequal cumulative pulse exposure, but no equal-exposure "
                "observation established recoverable temporal codeword identity."
            )
        else:
            verdict = "TEMPORAL_INFORMATION_NOT_SUPPORTED_AS_TESTED"
            claim = (
                "No observation established recoverable temporal codeword identity "
                "under the equal-exposure eligibility rule."
            )
    else:
        verdict = "TEMPORAL_INFORMATION_SURVIVAL_CANDIDATE_LOCATED"
        claim = (
            f"The exploratory ladder first recovered codeword identity under "
            f"equal cumulative pulse exposure at {level}, elapsed step {step} "
            f"(after_lag={after_lag}). This is a candidate boundary requiring "
    ...
    result = {
        "verdict": verdict,
        "bounded_claim": claim,
        "causal_onset_diagnostic_hits": causal_hits,
        "next_action": (
            "Freeze the first equal-exposure positive rung and observation time, "
            "then run an independent confirmatory version with larger groups and "
            "a predeclared endpoint."
            if level is not None
            else
            "Do not scale this exploratory design merely by sample size. Either "
            "change the temporal encoding/coupling so equal-exposure timing leaves "
            "a stronger persistent trace, or accept a negative Chapter 17 result."
        ),
        "explicit_nonclaims": [
            "semantics",
            "meaning",
...
EXPERIMENT_VERSION = "digital-crystal-information-survival-v3"
SCHEMA_VERSION = 2
...
def main():
    ...
    parser.add_argument(
        "--report-dir",
        type=Path,
        default=Path("research/digital-life/ch17-v3-reports"),
    )
    parser.add_argument(
        "--image-dir",
        type=Path,
    ...
    metadata = {
        ...
        "scientific_boundary": (
            "Exploratory boundary-finding for temporal codeword information under an "
            "equal-exposure eligibility rule. Unequal-prefix decoding is treated "
            "only as a causal-onset diagnostic. No semantics, sender identity, "
            "coordination, agency, individuality, life, or Shannon-capacity claim."
        ),
    ...
    print("=" * 78)
    print("CHAPTER 17 — WHAT SURVIVES THE CHANNEL? v3")
    ...
    fig = args.image_dir / "ch17-v3-05-information-boundary.png"
    ...
    metadata.update({
        ...
        "first_supported_after_lag": s5["first_supported_after_lag"],
