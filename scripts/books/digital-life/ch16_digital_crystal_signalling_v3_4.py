...
Experiment version: digital-crystal-signalling-v3.4
...
def stage_2_single_bit_intervention(
    ...
    capacity_guard = profile["max_capacity_fraction"]
    capacity_rows: List[dict] = []

    for rep in tqdm(range(nrep), desc="Single-bit interventions"):
        checkpoint_step = 12 + (rep % 8)

        # The environment must contain the full checkpoint prefix PLUS the
        # complete intervention horizon. v3.3 incorrectly used a fixed
        # `max(28, horizon + 18)`, which is one step too short for the largest
        # full-profile checkpoint (19 + 12 = 31 required vs 30 allocated).
        total_steps = checkpoint_step + horizon
        env = make_environment(total_steps, seed + 10_000 + rep * 101)
        base_seed = seed + 20_000 + rep * 103

        if len(env) < checkpoint_step + horizon:
            raise RuntimeError(
                "Stage 2 environment construction invariant failed: "
                f"len(env)={len(env)}, checkpoint_step={checkpoint_step}, "
                f"horizon={horizon}."
            )
        ...
        pulse_fraction = capacity_fraction(pulse_state, max_radius)
        no_fraction = capacity_fraction(no_state, max_radius)

        # Stage 2 is an exact paired intervention. If either branch reaches the
        # hard-radius saturation region, the endpoint comparison is not
        # interpretable and the replicate must not be silently retained.
        if max(pulse_fraction, no_fraction) >= capacity_guard:
            raise RuntimeError(
                "Stage 2 paired intervention reached hard-radius saturation: "
                f"rep={rep}, pulse_fraction={pulse_fraction:.3f}, "
                f"no_pulse_fraction={no_fraction:.3f}, "
                f"guard={capacity_guard:.3f}. Reduce checkpoint/horizon or "
                "increase radius."
            )
        ...
        capacity_rows.append({
            ...
            "pulse_capacity_fraction": float(pulse_fraction),
            "no_pulse_capacity_fraction": float(no_fraction),
        })
    ...
    result = {
        ...
        "saturation_guard": capacity_guard,
        "max_capacity_fraction_observed": float(
            max(
                max(r["pulse_capacity_fraction"], r["no_pulse_capacity_fraction"])
                for r in capacity_rows
            )
        ) if capacity_rows else 0.0,
        "capacity_diagnostics": capacity_rows,
...
def main() -> None:
    ...
    metadata = {
        ...
        "saturation_policy": (
            "Stages 1, 2, 3, 5, and 6 guard against hard-radius saturation. "
