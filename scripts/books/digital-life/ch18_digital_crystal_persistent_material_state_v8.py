...
class Reporter:
    ...
    def full_report(self, metadata: dict) -> Path:
        path = self.root / "ch18-persistent-material-state-v8-full-report.md"
        header = (
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "research/digital-life/ch18-persistent-material-state-v8"
        ),
    )
    parser.add_argument(
        "--image-dir",
    ...
    metadata = {
        ...
        "v7_supported_result": (
            "With exact copy quantity fixed, surface placement increased integrated "
            "causal accessibility, probability leverage, realized causal flips, "
            "and sustained causal lifetime."
        ),
        "v8_design": (
            "Restore endogenous opportunity feedback in one surface-biased branch "
            "and compare it with a surface-biased branch receiving a frozen "
            "exogenous propagation schedule generated from separate reference "
            "trajectories."
        ),
        "scientific_boundary": (
            "Opportunity feedback only. No claim of memory, learning, adaptation, "
    ...
    print("=" * 78)
    print("CHAPTER 18 V8 — OPPORTUNITY FEEDBACK")
    print(
    ...
    print(
        ...
        f"reference_groups={profile['reference_groups']}"
    )
    print("=" * 78)

    s0 = stage_0_reference_schedule(
        reporter,
        profile,
        crystal_params,
        args.seed,
    ...
    frozen_schedule = s0["_schedule_int_keys"]

    s1 = stage_1_feedback_mechanism_audit(
        reporter,
        profile,
        crystal_params,
        frozen_schedule,
    ...
    s2 = stage_2_feedback_vs_clamped(
        reporter,
        profile,
        crystal_params,
        frozen_schedule,
    ...
    print("\n" + "=" * 78)
    print("CHAPTER 18 V8 COMPLETE")
    print(f"reference_schedule={s0['status']}")
    print(f"feedback_mechanism={s1['status']}")
    print(f"feedback_vs_clamped={s2['status']}")
