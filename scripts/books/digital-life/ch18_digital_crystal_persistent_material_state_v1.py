...
PROFILES = {
    "quick": {
        ...
        # First experience / write.
        "experience_horizon": 8,
        "experience_pulse_step": 3,

        # Pulse-free retention interval after experience.
        "retention_delay": 6,
...
class Reporter:
    ...
    def full_report(self, metadata: dict) -> Path:
        path = self.root / "ch18-persistent-material-state-v1-full-report.md"
        header = (
            "# Chapter 18 — Can Experience Change the Material?\n\n"
            "## Run metadata\n\n```json\n"
...
def stage_1_write_and_persist(
    ...
    for g in tqdm(range(groups), desc="Stage 1 write/persistence"):
        total = warmup + exp_horizon + retention + 2
        ...
        future = env[warmup:]
        counts = []
        count_after_write = None

        for i in range(exp_horizon + retention):
            ...
            counts.append(len(state.modified))

            if i == pulse_step:
                count_after_write = len(state.modified)

        if count_after_write is None:
            raise RuntimeError("Experience pulse was not executed.")

        write_counts.append(count_after_write)
        retained_counts.append(len(state.modified))
        retained_fractions.append(
            len(state.modified) / max(1.0, len(state.occupied))
        )
        trajectories.append(counts)

    trajectory_mean = np.mean(np.asarray(trajectories, dtype=float), axis=0)

    write_summary = bootstrap_mean_ci(
        write_counts, profile["bootstrap_reps"], seed + 201_000
    )
    retained_summary = bootstrap_mean_ci(
        retained_counts, profile["bootstrap_reps"], seed + 202_000
    )
    fraction_summary = bootstrap_mean_ci(
        retained_fractions, profile["bootstrap_reps"], seed + 203_000
    )

    all_nonzero = all(x > 0 for x in write_counts)
    all_persisted_exactly = all(
        a == b for a, b in zip(write_counts, retained_counts)
    )

    result = {
        "experience_pulse_zero_index": pulse_step,
        "pulse_free_retention_steps": retention,
        "groups": groups,
        "modified_cells_immediately_after_write": write_summary,
        "modified_cells_after_retention_delay": retained_summary,
        "modified_fraction_after_retention_delay": fraction_summary,
        "all_groups_wrote_nonzero_state": all_nonzero,
        "all_groups_retained_exact_modified_count": all_persisted_exactly,
        "status": (
            "MEASURED"
            if all_nonzero and all_persisted_exactly
            else "FAILED"
        ),
        "bounded_statement": (
            "A transient pulse wrote local modified-cell state that remained "
            "present through the declared pulse-free retention interval. This "
            "establishes persistent experimental material state by construction; "
            "it does not establish memory."
        ),
    }

    reporter.json("stage-01-write-and-persist.json", result)
    reporter.stage(
        "stage-01-write-and-persist.md",
        "Stage 1 — Can Experience Write Persistent Local State?",
        f"```json\n{json.dumps(result, indent=2)}\n```",
    )

    fig, ax = plt.subplots(figsize=(9, 5))
    xs = np.arange(1, len(trajectory_mean) + 1)
    ax.plot(xs, trajectory_mean, marker="o")
    ax.axvline(pulse_step + 1, linestyle="--", linewidth=1)
    ax.set_xlabel("Elapsed step after warm checkpoint")
    ax.set_ylabel("Mean modified-cell count")
    ax.set_title("Experience-written material state across the retention interval")
    fig.tight_layout()
    image_dir.mkdir(parents=True, exist_ok=True)
    fig.savefig(
        image_dir / "ch18-v1-01-persistent-material-state.png",
        dpi=160,
    )
    plt.close(fig)
...
def stage_2_erase_ablation(
    ...
    for g in tqdm(range(groups), desc="Stage 2 erase ablation"):
        bundle = build_experience_checkpoints(
            profile, crystal_params, material_params, seed + 300_000, g
        )

        retained = bundle["experienced_retained"]
        erased = bundle["experienced_erased"]
        future = bundle["env_after_retention"]

        # Critical invariant: same visible morphology at ablation.
        if retained.occupied != erased.occupied:
            raise RuntimeError("Erase ablation changed visible morphology.")
        if retained.birth_time != erased.birth_time:
            raise RuntimeError("Erase ablation changed birth history.")
        if retained.stream_seed != erased.stream_seed:
            raise RuntimeError("Erase ablation changed CRN stream seed.")

        zero_bits = [0] * horizon

        A = advance_material_sequence(
    ...
    sym_summary = bootstrap_mean_ci(
        symdiff, profile["bootstrap_reps"], seed + 302_000
    )
    ...
    result = {
        "groups": groups,
        "primary_endpoint_steps_after_ablation": endpoint,
        "ablation": (
            "experienced retained material labels vs identical experienced "
            "morphology with labels erased"
        ),
        "visible_morphology_identical_at_ablation": True,
        "primary_test": test,
        "mean_pathwise_symmetric_difference": sym_summary,
        "alpha": profile["alpha"],
        "status": "SUPPORTED" if positive else "FAILED",
        "bounded_statement": (
            "Removing only the persistent material labels from an otherwise "
            "identical experienced crystal "
            + (
                "changed later growth systematically under the frozen paired "
                "protocol."
                if positive
                else "did not establish a systematic later-growth difference "
                "under the frozen paired protocol."
            )
        ),
    }

    reporter.json("stage-02-erase-ablation.json", result)
    reporter.stage(
        "stage-02-erase-ablation.md",
        "Stage 2 — Does the Retained State Causally Alter Later Growth?",
        f"```json\n{json.dumps(result, indent=2)}\n```",
    )
    return result
...
# ============================================================================
# Stage 3 — does retained experience change response to a later pulse?
# ============================================================================

def stage_3_reexposure_response(
    reporter: Reporter,
    profile: dict,
    crystal_params: CrystalParams,
    material_params: MaterialParams,
    ...
    challenge_step = profile["challenge_pulse_step"]

    # Difference-in-differences response vectors.
    retained_deltas = {t: [] for t in observations}
    erased_deltas = {t: [] for t in observations}
    naive_deltas = {t: [] for t in observations}

    retained_vs_erased_state_sym = []
    erased_vs_naive_state_sym = []

    pulse_bits = [0] * horizon
    pulse_bits[challenge_step] = 1
    zero_bits = [0] * horizon

    for g in tqdm(range(groups), desc="Stage 3 re-exposure response"):
        bundle = build_experience_checkpoints(
            profile, crystal_params, material_params, seed + 400_000, g
        )
        ...
        # Retained and erased must be identical except for material labels.
        if retained.occupied != erased.occupied:
            raise RuntimeError(
                "Retained/erased challenge checkpoints differ in morphology."
            )

        retained_vs_erased_state_sym.append(
            normalized_symmetric_difference(
                retained.occupied, erased.occupied
            )
        )
        erased_vs_naive_state_sym.append(
            normalized_symmetric_difference(
                erased.occupied, naive.occupied
            )
        )
        ...
        for name, cp in (
            ...
            branches[(name, "pulse")] = advance_material_sequence(
                cp, future, pulse_bits,
                profile["message_gain"], radius,
                crystal_params, material_params,
                observations, profile["max_capacity_fraction"],
            )
            branches[(name, "no_pulse")] = advance_material_sequence(
                cp, future, zero_bits,
                profile["message_gain"], radius,
                crystal_params, material_params,
                observations, profile["max_capacity_fraction"],
        ...
        for t in observations:
            retained_deltas[t].append(
                response_delta(
                    branches[("retained", "pulse")][t],
                    branches[("retained", "no_pulse")][t],
                    radius,
                )
            )
            erased_deltas[t].append(
                response_delta(
                    branches[("erased", "pulse")][t],
                    branches[("erased", "no_pulse")][t],
                    radius,
                )
            )
            naive_deltas[t].append(
                response_delta(
                    branches[("naive", "pulse")][t],
                    branches[("naive", "no_pulse")][t],
                    radius,
                )
            )
    ...
    for t in observations:
        R = np.asarray(retained_deltas[t], dtype=float)
        E = np.asarray(erased_deltas[t], dtype=float)
        N = np.asarray(naive_deltas[t], dtype=float)

        # Primary causal contrast: same experienced morphology, labels retained
        # versus erased. This isolates the experimental material state.
        retained_vs_erased = paired_ridge_test(
            R, E,
            permutations=profile["permutations"],
            seed=seed + 410_000 + t,
        )

        # Secondary descriptive contrast: experienced morphology with labels
        # erased versus never-experienced morphology.
        erased_vs_naive = paired_ridge_test(
            E, N,
            permutations=profile["permutations"],
            seed=seed + 420_000 + t,
        )
        ...
        results[str(t)] = {
            "retained_vs_erased_primary": retained_vs_erased,
            "erased_vs_naive_secondary": erased_vs_naive,
            "mean_response_norm_retained": bootstrap_mean_ci(
                np.linalg.norm(R, axis=1),
                profile["bootstrap_reps"],
                seed + 430_000 + t,
            ),
            "mean_response_norm_erased": bootstrap_mean_ci(
                np.linalg.norm(E, axis=1),
                profile["bootstrap_reps"],
                seed + 440_000 + t,
            ),
            "mean_response_norm_naive": bootstrap_mean_ci(
    ...
    primary = results[str(endpoint)]["retained_vs_erased_primary"]
    positive = bool(primary["p_value"] < profile["alpha"])

    result = {
        "groups": groups,
        "challenge_pulse_zero_index": challenge_step,
        "observation_steps": observations,
        "primary_endpoint": endpoint,
        "primary_contrast": (
            "difference in pulse response between experienced-retained and "
            "experienced-erased crystals"
        ),
        "primary_test": primary,
        "results": results,
        "checkpoint_control": {
            "retained_vs_erased_visible_symdiff": bootstrap_mean_ci(
                retained_vs_erased_state_sym,
                profile["bootstrap_reps"],
                seed + 460_000,
            ),
            "erased_vs_naive_visible_symdiff": bootstrap_mean_ci(
                erased_vs_naive_state_sym,
                profile["bootstrap_reps"],
                seed + 461_000,
            ),
        ...
        "bounded_statement": (
            "Past experience altered a persistent local material state, and "
            + (
                "retaining that state changed the receiver's response to a later "
                "identical pulse relative to erasing the state while holding "
                "experienced morphology fixed."
                if positive
                else "the frozen experiment did not establish that retaining "
                "that state changed the receiver's response to a later identical "
                "pulse relative to erasing it while holding experienced "
                "morphology fixed."
            )
        ),
    }

    reporter.json("stage-03-reexposure-response.json", result)
    reporter.stage(
        "stage-03-reexposure-response.md",
...
def stage_4_verdict(
    ...
    if not (
        ...
        next_question = "Fix extension validity before interpreting material state."
    elif stage1["status"] != "MEASURED":
        final_status = "FAILED"
        next_question = "The write mechanism did not reliably create persistent state."
    elif stage2["status"] != "SUPPORTED":
        final_status = "FAILED"
        next_question = (
            "Persistent labels existed but were not shown to causally alter later "
            "growth under the ablation protocol."
        )
    elif stage3["status"] == "PROVISIONAL":
        final_status = "PROVISIONAL"
        next_question = (
            "Can the persistent material state be updated or overwritten by later "
            "experience rather than only written irreversibly once?"
        )
    else:
        final_status = "SUPPORTED"
        next_question = (
            "The retained material state altered later growth, but modulation of "
            "a second pulse response was not established. Characterize how and "
            "when the state influences dynamics before claiming adaptation."
        )
    ...
    result = {
        ...
        "stage_1_persistent_state": stage1["status"],
        "stage_2_material_state_causal": stage2["status"],
        "stage_3_later_response_changed": stage3["status"],
        "final_status": final_status,
        "bounded_claim": (
            "This experiment tests whether a transient pulse can write a local "
            "persistent material state, whether that state is causally active, "
            "and whether it changes response to later perturbation. It does not "
            "by itself establish memory, learning, adaptation, signalling, "
            "semantics, agency, individuality, reproduction, or life."
        ),
        "nonclaims": [
            "memory",
            "learning",
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "research/digital-life/ch18-persistent-material-state-v1"
        ),
    )
    parser.add_argument(
        "--image-dir",
    ...
    metadata = {
        ...
        "experimental_extension": {
            ...
            "write_rule": (
                "During an explicit pulse, each occupied boundary cell that is "
                "not already modified is independently written with fixed "
                "probability using a separate cell-keyed material RNG channel."
            ),
            "persistence_rule": "modified remains modified",
            "causal_rule": (
                "each modified occupied neighbour adds a fixed local growth-score "
                "bias to a frontier cell on later steps"
            ),
            "write_probability": material_params.write_probability,
            "modified_neighbor_gain": material_params.modified_neighbor_gain,
        },
        "scientific_boundary": (
            "Persistent local material state only. No global memory register, "
            "history buffer, learned parameter, target behaviour, or decoder."
        ),
        "started_at_unix": time.time(),
    }

    print("=" * 78)
    print("CHAPTER 18 — CAN EXPERIENCE CHANGE THE MATERIAL?")
    print(
    ...
    s1 = stage_1_write_and_persist(
        reporter, profile, crystal_params, material_params,
        args.seed, args.image_dir,
    )

    s2 = stage_2_erase_ablation(
        reporter, profile, crystal_params, material_params, args.seed
    )

    s3 = stage_3_reexposure_response(
        reporter, profile, crystal_params, material_params, args.seed
    )
    ...
    print("\n" + "=" * 78)
    print("CHAPTER 18 V1 COMPLETE")
    print(f"persistent_state={s1['status']}")
    print(f"material_state_causal={s2['status']}")
