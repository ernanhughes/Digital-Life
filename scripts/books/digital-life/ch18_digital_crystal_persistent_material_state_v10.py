...
EXPERIMENT_VERSION = "digital-crystal-persistent-material-state-v10"
SCHEMA_VERSION = 10
...
def safe_ratio(
    ...
    """
    Return numerator / denominator, with zero when denominator is non-positive.

    In V10 the denominators are causal-opportunity quantities such as contact AUC
    or applied transmission count. A zero denominator means no opportunity was
    available, so efficiency is defined as zero rather than undefined.
    """
...
def group_efficiency_metrics(
    contact_count_auc: float,
    probability_leverage_auc: float,
    total_realized_flips: float,
    applied_transmissions: float,
) -> dict:
    """
    Primary V10 normalized causal-productivity measures.
    """
    return {
        "leverage_per_contact": safe_ratio(
            probability_leverage_auc,
            contact_count_auc,
        ),
        "flips_per_contact": safe_ratio(
            total_realized_flips,
            contact_count_auc,
        ),
        "flips_per_transmission": safe_ratio(
            total_realized_flips,
...
class Reporter:
    ...
    def full_report(self, metadata: dict) -> Path:
        path = self.root / "ch18-persistent-material-state-v10-full-report.md"
        header = (
            "# Chapter 18 — Can Experience Change the Material? (V10 Temporal Causal Efficiency)\n\n"
            "## Run metadata\n\n```json\n"
...
def stage_2_temporal_causal_efficiency(
    reporter: Reporter,
    profile: dict,
    crystal_params: CrystalParams,
    schedules: dict,
    ...
    """
    Primary V10 experiment.

    Replay ALIGNED / SHUFFLED / SHIFTED schedules exactly as in v9, then measure
    normalized causal productivity rather than gross causal lifetime.

    Primary metrics:
        leverage_per_contact
            probability_leverage_auc / contact_count_auc

        flips_per_contact
            total_realized_flips / contact_count_auc

        flips_per_transmission
            total_realized_flips / applied_transmissions

    Prediction:
        ALIGNED > SHUFFLED
        ALIGNED > SHIFTED

    for all three primary metrics.

    Access AUC, contact AUC, leverage AUC, flips, and sustained-loss time remain
    descriptive mechanism context and cannot rescue or overturn the primary
    efficiency decision.
    """
    groups = profile["groups"]
    radius = profile["radius"]
    ...
    truncations = {b: [] for b in branches}

    for g in tqdm(range(groups), desc="V10 temporal causal efficiency"):
        run = run_timing_triplet(
            profile,
            crystal_params,
            schedules,
        ...
        for branch in branches:
            ...
            access_auc = discrete_auc(frac_by_t, start, end)
            contact_auc = discrete_auc(contact_by_t, start, end)
            leverage_auc = discrete_auc(leverage_by_t, start, end)
            total_flips = discrete_auc(flips_by_t, start, end)
            loss_time = sustained_zero_loss_time(
                contact_by_t,
                start,
                end,
                profile["sustained_zero_steps"],
            )
            applied_tx = sum(
                run["logs"][t - 1][branch]["applied_budget"]
                for t in range(start, end + 1)
            )

            raw[branch]["access_fraction_auc"].append(access_auc)
            raw[branch]["contact_count_auc"].append(contact_auc)
            raw[branch]["probability_leverage_auc"].append(leverage_auc)
            raw[branch]["total_realized_flips"].append(total_flips)
            raw[branch]["sustained_loss_time"].append(loss_time)
            raw[branch]["applied_transmissions"].append(applied_tx)

            eff = group_efficiency_metrics(
                contact_count_auc=contact_auc,
                probability_leverage_auc=leverage_auc,
                total_realized_flips=total_flips,
                applied_transmissions=applied_tx,
            )
            for key, value in eff.items():
                efficiency[branch][key].append(value)

            truncations[branch].extend(
                run["logs"][t - 1][branch]["truncated"]
                for t in range(start, end + 1)
            )
    ...
    for bi, branch in enumerate(branches):
        raw_summary[branch] = {
            ...
            for j, (key, vals) in enumerate(raw[branch].items())
        }
        raw_summary[branch]["truncation_rate"] = float(
            np.mean(truncations[branch])
        )

        efficiency_summary[branch] = {
            key: bootstrap_mean_ci(
                vals,
                profile["bootstrap_reps"],
                seed + 315_000 + bi * 1000 + j,
            )
            for j, (key, vals) in enumerate(efficiency[branch].items())
        }
    ...
    for i, metric in enumerate(primary_metrics):
        paired_tests[metric] = {
            "aligned_gt_shuffled": paired_randomization_difference_test(
                efficiency["shuffled"][metric],
                efficiency["aligned"][metric],
                profile["permutations"],
                seed + 320_000 + i * 100,
                alternative="greater",
            ),
            "aligned_gt_shifted": paired_randomization_difference_test(
                efficiency["shifted"][metric],
                efficiency["aligned"][metric],
                profile["permutations"],
                seed + 330_000 + i * 100,
                alternative="greater",
    ...
    for metric in primary_metrics:
        aligned = np.asarray(efficiency["aligned"][metric], dtype=float)
        shuffled = np.asarray(efficiency["shuffled"][metric], dtype=float)
        shifted = np.asarray(efficiency["shifted"][metric], dtype=float)

        per_group_direction[metric] = {
            "aligned_gt_shuffled_fraction": float(
                np.mean(aligned > shuffled)
            ),
            "aligned_gt_shifted_fraction": float(
                np.mean(aligned > shifted)
            ),
            "aligned_gt_both_fraction": float(
                np.mean((aligned > shuffled) & (aligned > shifted))
            ),
        }
    ...
    result = {
        ...
        "raw_mechanism_summary": raw_summary,
        "efficiency_summary": efficiency_summary,
        "paired_tests": paired_tests,
        "per_group_direction": per_group_direction,
        "trajectory_summary": trajectory_summary,
        "status": "MEASURED",
        "bounded_statement": (
            "V10 asks whether temporal arrangement changes causal productivity "
            "per unit of accessible frontier opportunity and per realized "
            "transmission, rather than whether it increases gross accessibility."
        ),
    }

    reporter.json("stage-02-temporal-causal-efficiency.json", result)
    reporter.stage(
        "stage-02-temporal-causal-efficiency.md",
        "Stage 2 — Does Timing Change Causal Efficiency?",
        f"```json\n{json.dumps(result, indent=2)}\n```",
    )

    # Evidence plot 1: flips per contact.
    fig, ax = plt.subplots(figsize=(8, 5))
    xs = list(range(len(branches)))
    ys = [
        efficiency_summary[b]["flips_per_contact"]["mean"]
        for b in branches
    ]
    ax.bar(xs, ys)
    ax.set_xticks(xs)
    ax.set_xticklabels(branches)
    ax.set_ylabel("Mean realized flips / contact-count AUC")
    ax.set_title("V10 temporal causal efficiency")
    fig.tight_layout()

    image_dir.mkdir(parents=True, exist_ok=True)
    fig.savefig(
        image_dir / "ch18-v10-01-flips-per-contact.png",
        dpi=160,
    )
    plt.close(fig)

    # Evidence plot 2: raw access trajectory retained only for context.
    fig, ax = plt.subplots(figsize=(9, 5))
    xs = list(range(start, end + 1))
    for branch in branches:
        ys = [
    ...
    ax.set_xlabel("Elapsed step after warm checkpoint")
    ax.set_ylabel("Mean frontier fraction exposed to modified material")
    ax.set_title("V10 accessibility context (not primary endpoint)")
    ax.legend()
    fig.tight_layout()
    fig.savefig(
        image_dir / "ch18-v10-02-access-context.png",
        dpi=160,
    )
    plt.close(fig)
...
def stage_3_efficiency_checkpoints(
    reporter: Reporter,
    profile: dict,
    stage2: dict,
) -> dict:
    """
    Secondary raw checkpoints only.

    These values help interpret the normalized efficiency result, but cannot
    change the primary V10 decision.
    """
    result = {
        "role": "SECONDARY DESCRIPTIVE",
        "steps": profile["descriptive_steps"],
        "results": {
            str(t): {
                branch: stage2["trajectory_summary"][branch][str(t)]
                for branch in ("aligned", "shuffled", "shifted")
            }
    ...
    reporter.json("stage-03-efficiency-checkpoints.json", result)
    reporter.stage(
        "stage-03-efficiency-checkpoints.md",
        "Stage 3 — Secondary Raw Timing Checkpoints",
        f"```json\n{json.dumps(result, indent=2)}\n```",
    )
    return result
...
def stage_4_efficiency_verdict(
    reporter: Reporter,
    profile: dict,
    stage0: dict,
    stage1: dict,
    ...
    Required:
      1. v9 timing replay requested-quantity invariant remains valid.
      2. applied-copy imbalance remains acceptably small.
      3. aligned beats BOTH shuffled and shifted on all three primary
         normalized efficiency metrics at alpha.

    Gross access AUC is explicitly NOT required.
    """
    alpha = profile["alpha"]
    ...
    requested_quantity_valid = bool(
        stage0["exact_same_multiset_and_total"]
        and stage1["requested_totals_exactly_equal"]
    )

    all_primary_pass = all(
        stage2["paired_tests"][metric][
            "aligned_gt_shuffled"
        ]["p_value"] < alpha
        and stage2["paired_tests"][metric][
            "aligned_gt_shifted"
        ]["p_value"] < alpha
        for metric in primary_metrics
    )

    raw_summary = stage2["raw_mechanism_summary"]

    max_truncation = max(
        raw_summary[b]["truncation_rate"]
        for b in ("aligned", "shuffled", "shifted")
    )
    truncation_ok = max_truncation <= 0.10

    applied_means = {
        b: raw_summary[b]["applied_transmissions"]["mean"]
        for b in ("aligned", "shuffled", "shifted")
    }
    applied_mean = float(np.mean(list(applied_means.values())))
    applied_spread = max(applied_means.values()) - min(applied_means.values())
    applied_relative_spread = (
        applied_spread / applied_mean if applied_mean > 0 else 0.0
    )
    applied_balance_ok = applied_relative_spread <= 0.05

    if (
        requested_quantity_valid
        and all_primary_pass
        and truncation_ok
        and applied_balance_ok
    ):
        status = "SUPPORTED"
        bounded_claim = (
            "With donor-derived requested copy quantity and surface placement "
            "held fixed, aligned temporal replay produced greater causal leverage "
            "per frontier contact, more realized causal flips per frontier "
            "contact, and more realized causal flips per applied transmission "
            "than shuffled or shifted replay under this protocol."
        )

    elif requested_quantity_valid and all_primary_pass:
        status = "PROVISIONAL"
        bounded_claim = (
            "Aligned timing outperformed shuffled and shifted timing on all "
            "predeclared normalized causal-efficiency metrics, but realized copy "
            "imbalance or schedule truncation weakened a timing-only claim."
        )

    else:
        status = "FAILED"
        bounded_claim = (
            "V10 did not establish that aligned temporal organization improves "
            "causal efficiency independently of requested copy quantity and "
            "surface placement."
        )

    result = {
        "experiment_role": "EXPLORATORY TEMPORAL CAUSAL-EFFICIENCY TEST",
        "chapter": 18,
        "question": (
            "Does temporal organization change how much causal work propagated "
            "state performs per unit of accessible opportunity?"
        ),
        "primary_metrics": list(primary_metrics),
        "requested_quantity_invariant_valid": requested_quantity_valid,
        "all_primary_efficiency_tests_pass": all_primary_pass,
        "gross_access_is_not_a_primary_requirement": True,
        "max_truncation_rate": max_truncation,
        "truncation_rate_acceptable": truncation_ok,
        "applied_transmission_means": applied_means,
        "applied_relative_spread": applied_relative_spread,
        ...
        "next_question": (
            "If temporal causal efficiency is supported, introduce a second "
            "experience and test whether the material dynamics can alter which "
            "timing pattern is expressed, before using memory or adaptation "
            "terminology."
        ),
    }

    reporter.json("stage-04-efficiency-verdict.json", result)
    reporter.stage(
        "stage-04-efficiency-verdict.md",
        "Stage 4 — Bounded Chapter 18 V10 Verdict",
        f"```json\n{json.dumps(result, indent=2)}\n```",
    )
    return result
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "research/digital-life/ch18-persistent-material-state-v10"
        ),
    )
    parser.add_argument(
        "--image-dir",
    ...
    metadata = {
        ...
        "run_type": "EXPLORATORY TEMPORAL CAUSAL-EFFICIENCY TEST",
        "profile": args.profile,
        "profile_config": profile,
        "seed": args.seed,
        "canonical_model_modified": False,
        "counterfactual_coupling": "cell-keyed CRN from Chapter 17",
        "v9_result_being_followed": (
            "V9 failed the broad temporal-lifetime hypothesis: aligned timing "
            "did not reliably improve gross access AUC. However, aligned timing "
            "did improve probability-leverage AUC and realized causal flips "
            "relative to shuffled and shifted replay."
        ),
        "v10_design": (
            "Keep the exact donor-derived aligned/shuffled/shifted replay control "
            "from v9, but make normalized causal productivity the primary outcome: "
            "leverage per frontier contact, realized flips per frontier contact, "
            "and realized flips per applied transmission."
        ),
        "scientific_boundary": (
            "Causal efficiency only. No claim of memory, learning, adaptation, "
            "self-maintenance, homeostasis, autopoiesis, or life."
        ),
        "started_at_unix": time.time(),
    }

    print("=" * 78)
    print("CHAPTER 18 V10 — TEMPORAL CAUSAL EFFICIENCY")
    print(
    ...
    # Reuse the exact donor schedule generation and replay-integrity audit from v9.
    ...
    s2 = stage_2_temporal_causal_efficiency(
        reporter,
        profile,
        crystal_params,
        schedules,
    ...
    s3 = stage_3_efficiency_checkpoints(
        reporter,
        profile,
        s2,
    )

    s4 = stage_4_efficiency_verdict(
        reporter,
        profile,
        s0,
        s1,
