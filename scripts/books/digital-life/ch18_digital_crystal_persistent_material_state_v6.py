...
class Reporter:
    ...
    def full_report(self, metadata: dict) -> Path:
        path = self.root / "ch18-persistent-material-state-v6-full-report.md"
        header = (
...
def stage_0_extension_audit(
    ...
    """
    Validate exact synchronized-budget logic and reproducibility.
    """
    ...
    equal_counts = len(set(selected_counts.values())) == 1
    all_equal_budget = all(v == budget for v in selected_counts.values())

    if not equal_counts or not all_equal_budget:
        raise RuntimeError("V6 synchronized budget unit audit failed.")
    ...
    result = {
        ...
        "synthetic_eligible_counts": {
            p: len(synthetic_eligible[p]) for p in policies
        },
        "shared_budget": budget,
        "selected_counts": selected_counts,
        "equal_selected_count": equal_counts,
        "all_selected_counts_equal_shared_budget": all_equal_budget,
        "scientific_role": (
            "All placement policies receive exactly the same transmission count "
            "at every synchronized propagation step. The experiment isolates "
            "spatial allocation rather than realized copy quantity."
        ),
    }

    reporter.json("stage-00-v6-audit.json", result)
    reporter.stage(
        "stage-00-v6-audit.md",
...
def run_synchronized_policy_trajectory(
    ...
    """
    Run interior/random/surface branches side-by-side.

    Each propagation step:
      1. grow all branches independently under shared environment + CRN;
      2. compute each branch's eligible propagation targets;
      3. compute ONE shared K from min eligible count;
      4. allocate exactly K targets per branch using branch policy;
    ...
    for i in range(max_elapsed):
        ...
        # No propagation on the direct pulse-write step itself. Propagation
        # begins on later growth steps.
        ...
        if len(set(per_step_counts)) != 1:
            raise RuntimeError(
                f"V6 invalid: per-step transmission counts diverged at "
                f"elapsed={i+1}: {per_step_counts}"
            )

        if len(set(cumulative.values())) != 1:
            raise RuntimeError(
                f"V6 invalid: cumulative transmission counts diverged at "
...
def stage_1_budget_integrity_and_placement(
    reporter: Reporter,
    profile: dict,
    crystal_params: CrystalParams,
    seed: int,
    ...
    cumulative_by_policy = {p: [] for p in policies}
    selected_surface_by_policy = {p: [] for p in policies}
    eligible_by_policy = {p: [] for p in policies}
    per_group_integrity = []

    for g in tqdm(
        range(groups),
        desc="Stage 1 synchronized budget audit",
    ):
        ...
        final_counts = run["final_cumulative"]
        integrity = len(set(final_counts.values())) == 1
        per_group_integrity.append(integrity)

        for p in policies:
            cumulative_by_policy[p].append(final_counts[p])

        for row in run["transmission_log"]:
            for p in policies:
                eligible_by_policy[p].append(
                    row["eligible_counts"][p]
                )
                selected_surface_by_policy[p].append(
                    row["mean_selected_surface_exposure"][p]
                )

    if not all(per_group_integrity):
        raise RuntimeError(
            "V6 invalid: matched cumulative budget failed in at least one group."
        )

    summary = {}
    for pi, p in enumerate(policies):
        summary[p] = {
            "cumulative_transmissions": bootstrap_mean_ci(
                cumulative_by_policy[p],
                profile["bootstrap_reps"],
                seed + 210_000 + pi * 100,
            ),
            "eligible_count": bootstrap_mean_ci(
                eligible_by_policy[p],
                profile["bootstrap_reps"],
                seed + 211_000 + pi * 100,
            ),
            "mean_selected_surface_exposure": bootstrap_mean_ci(
                selected_surface_by_policy[p],
                profile["bootstrap_reps"],
                seed + 212_000 + pi * 100,
            ),
        }
    ...
    result = {
        "groups": groups,
        "placement_policies": policies,
        "all_groups_exact_cumulative_budget_match": all(
            per_group_integrity
        ),
        "summary": summary,
        "status": "MEASURED",
        "bounded_statement": (
            "Every synchronized branch transmitted exactly the same number of "
            "cells at every step and therefore the same cumulative number of "
            "cells. Differences between policies are attributable to placement "
            "and its downstream consequences, not realized copy count."
        ),
    }
...
def stage_2_placement_ordering(
    reporter: Reporter,
    profile: dict,
    crystal_params: CrystalParams,
    seed: int,
    ...
    for g in tqdm(
        range(groups),
        desc="Stage 2 matched-budget placement ordering",
    ):
        run = run_synchronized_policy_trajectory(
            profile,
            crystal_params,
        ...
        for p in policies:
            for t in steps:
                m = material_accessibility_metrics(
                    run["states"][p][t],
                    radius,
                )
                for key in metrics[p][t]:
                    metrics[p][t][key].append(m[key])
        ...
        for t in steps:
            summary[p][str(t)] = {
                key: bootstrap_mean_ci(
                    vals,
                    profile["bootstrap_reps"],
                    seed + 310_000 + pi * 10000 + t * 100 + j,
                )
                for j, (key, vals) in enumerate(metrics[p][t].items())
            }
    ...
    expected_order = (
        late_contact["interior_biased"]
        < late_contact["random_matched"]
        < late_contact["surface_biased"]
    )
    ...
    result = {
        ...
        "summary": summary,
        "late_step_frontier_contact": late_contact,
        "interior_less_than_random_less_than_surface": expected_order,
        "status": "MEASURED",
        "bounded_statement": (
            "V6 measures whether equal quantities of propagated state remain "
            "differentially accessible depending only on spatial placement."
        ),
    }

    reporter.json("stage-02-placement-ordering.json", result)
    reporter.stage(
        "stage-02-placement-ordering.md",
    ...
    for p in policies:
        ys = [
            summary[p][str(t)]["frontier_exposed_fraction"]["mean"]
            for t in steps
        ]
        ax.plot(steps, ys, marker="o", label=p)

    ax.set_xlabel("Elapsed step after warm checkpoint")
    ax.set_ylabel("Mean frontier fraction exposed to modified material")
    ax.set_title("Exact matched-cumulative-budget placement comparison")
    ax.legend()
    fig.tight_layout()

    image_dir.mkdir(parents=True, exist_ok=True)
    fig.savefig(
        image_dir / "ch18-v6-01-placement-ordering.png",
        dpi=160,
    )
    plt.close(fig)
...
def stage_3_cell_level_causal_ordering(
    reporter: Reporter,
    profile: dict,
    crystal_params: CrystalParams,
    seed: int,
    ...
    for g in tqdm(
        range(groups),
        desc="Stage 3 cell-level causal ordering",
    ):
        run = run_synchronized_policy_trajectory(
            profile,
            crystal_params,
        ...
        for p in policies:
            ...
            for t in audit_steps:
                ...
                for key in metrics[p][t]:
                    metrics[p][t][key].append(audit[key])
    ...
    target = str(profile["late_ablation_step"])
    late_flips = {
        p: summary[p][target]["realized_causal_flips"]["mean"]
        for p in policies
    }
    late_sum_delta = {
        p: summary[p][target]["sum_delta_p"]["mean"]
        for p in policies
    }

    flip_order = (
        late_flips["interior_biased"]
        < late_flips["random_matched"]
        < late_flips["surface_biased"]
    )
    ...
    result = {
        ...
        "summary": summary,
        "late_step_realized_flips": late_flips,
        "late_step_sum_delta_p": late_sum_delta,
        "interior_less_than_random_less_than_surface_flips": flip_order,
        "status": "MEASURED",
        "bounded_statement": (
            "Under identical cumulative transmission counts, V6 tests whether "
            "placement alone changes the amount of local causal work performed "
            "at the active frontier."
        ),
    }

    reporter.json("stage-03-cell-causal-ordering.json", result)
    reporter.stage(
        "stage-03-cell-causal-ordering.md",
...
def stage_4_late_ablation_ordering(
    reporter: Reporter,
    profile: dict,
    crystal_params: CrystalParams,
    seed: int,
    ...
    for g in tqdm(
        range(groups),
        desc="Stage 4 late ablation ordering",
    ):
        run = run_synchronized_policy_trajectory(
            profile,
            crystal_params,
    ...
    for pi, p in enumerate(policies):
        X = np.asarray(results[p]["X"], dtype=float)
        Y = np.asarray(results[p]["Y"], dtype=float)

        out[p] = {
            "frontier_contact": bootstrap_mean_ci(
                results[p]["contact"],
                profile["bootstrap_reps"],
                seed + 510_000 + pi * 100,
            ),
            "pathwise_symmetric_difference": bootstrap_mean_ci(
                results[p]["symdiff"],
                profile["bootstrap_reps"],
                seed + 511_000 + pi * 100,
            ),
            "paired_ridge_test": paired_ridge_test(
                X,
                Y,
                permutations=profile["permutations"],
                seed=seed + 512_000 + pi * 100,
            ),
        }

    means = {
        p: out[p]["pathwise_symmetric_difference"]["mean"]
        for p in policies
    }

    ordered = (
        means["interior_biased"]
        < means["random_matched"]
        < means["surface_biased"]
    )
    ...
    result = {
        ...
        "results": out,
        "interior_less_than_random_less_than_surface_symdiff": ordered,
        "status": "MEASURED",
        "interpretation": (
            "This is a downstream corroboration of the exact matched-budget "
            "placement experiment."
        ),
    }

    reporter.json("stage-04-late-ablation-ordering.json", result)
    reporter.stage(
        "stage-04-late-ablation-ordering.md",
...
def stage_5_verdict(
    ...
    budget_valid = bool(
        stage0["equal_selected_count"]
        and stage0["all_selected_counts_equal_shared_budget"]
        and stage1["all_groups_exact_cumulative_budget_match"]
    )

    access_order = bool(
        stage2["interior_less_than_random_less_than_surface"]
    )
    flip_order = bool(
        stage3["interior_less_than_random_less_than_surface_flips"]
    )
    ablation_order = bool(
        stage4["interior_less_than_random_less_than_surface_symdiff"]
    )

    if not budget_valid:
        status = "UNTESTED"
        bounded = (
            "The exact matched-budget invariant failed, so placement cannot be "
            "interpreted independently of copy quantity."
        )

    elif access_order and flip_order and ablation_order:
        status = "SUPPORTED"
        bounded = (
            "With exactly matched per-step and cumulative transmission counts, "
            "placing propagated state toward the active surface produced more "
            "late frontier accessibility, more realized local causal flips, and "
            "a larger late erase effect than random placement, while interior "
            "placement produced less."
        )

    elif access_order and flip_order:
        status = "PROVISIONAL"
        bounded = (
            "Exact matched-budget placement produced the predicted ordering in "
            "frontier accessibility and realized local causal work, but the "
            "whole-crystal late ablation ordering did not fully align."
        )

    else:
        status = "FAILED"
        bounded = (
            "Under an exact matched cumulative copying budget, V6 did not "
            "establish the predicted interior < random < surface ordering."
        )
    ...
    result = {
        ...
        "chapter": 18,
        "question": (
            "Does the spatial placement of persistent material determine whether "
            "it remains causally active when the exact amount copied is held fixed?"
        ),
        "budget_invariant_valid": budget_valid,
        "predicted_order": "interior_biased < random_matched < surface_biased",
        "frontier_access_order_supported": access_order,
        "realized_causal_flip_order_supported": flip_order,
        "late_ablation_order_supported": ablation_order,
        "status": status,
        ...
        "next_question": (
            "If exact-budget surface placement is supported, test whether a later "
            "experience can update, replace, or compete with the existing "
            "surface-associated state without increasing the propagation budget."
        ),
    }
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "research/digital-life/ch18-persistent-material-state-v6"
        ),
    )
    parser.add_argument(
        "--image-dir",
    ...
    metadata = {
        ...
        "v5_issue_being_fixed": (
            "V5 matched transmission fraction but not realized copy quantity. "
            "Surface placement created more eligible opportunities and therefore "
            "more cumulative transmissions."
        ),
        "v6_design": (
            "Run interior, random, and surface placement side-by-side. At every "
            "step compute one shared K from the minimum eligible count and force "
            "all branches to transmit exactly K cells. Assert equal cumulative "
            "transmission counts after every step."
        ),
        "scientific_boundary": (
            "Placement-only mechanism test. No claim of memory, learning, "
            "adaptation, self-maintenance, homeostasis, or life."
        ),
        "started_at_unix": time.time(),
    }

    print("=" * 78)
    print("CHAPTER 18 V6 — EXACT MATCHED-CUMULATIVE-BUDGET PLACEMENT")
    print(
        f"profile={args.profile} "
        f"version={EXPERIMENT_VERSION} "
        f"groups={profile['groups']}"
    )
    print(f"policies={profile['placement_policies']}")
    ...
    s1 = stage_1_budget_integrity_and_placement(
        reporter, profile, crystal_params, args.seed
    )

    s2 = stage_2_placement_ordering(
        reporter, profile, crystal_params,
        args.seed, args.image_dir,
    )

    s3 = stage_3_cell_level_causal_ordering(
        reporter, profile, crystal_params, args.seed
    )

    s4 = stage_4_late_ablation_ordering(
        reporter, profile, crystal_params, args.seed
    )
    ...
    print("\n" + "=" * 78)
    print("CHAPTER 18 V6 COMPLETE")
    print(f"budget_integrity={s1['status']}")
    print(f"placement_ordering={s2['status']}")
    print(f"cell_causal_ordering={s3['status']}")
    print(f"late_ablation_ordering={s4['status']}")
