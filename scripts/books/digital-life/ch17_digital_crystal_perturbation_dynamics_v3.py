...
class Reporter:
    ...
    def full_report(self, metadata: dict) -> Path:
        path = self.root / "ch17-perturbation-dynamics-v3-full-report.md"
        header = (
            "# Chapter 17 — How Does the Crystal Respond to Perturbation?\n\n"
            "## Run metadata\n\n```json\n"
...
def stage_3_matched_arrangement(reporter, profile, params, seed, image_dir):
    ...
    fig.tight_layout()
    fig.savefig(image_dir / "ch17-v3-03-matched-arrangement.png", dpi=160)
    plt.close(fig)
...
# ---------------------------------------------------------------------------
# Stage 5 — measurement-instrument calibration
# ---------------------------------------------------------------------------
...
def stage_5_measurement_resolution(
    ...
    """
    Compare candidate measurement instruments without using the real A/B labels
    to choose among them.

    Synthetic paired experiments are built from:
      - a real baseline feature vector A from the Stage 3 A arm;
      - an independently measured no-pulse stochastic feature-difference vector
        from Stage 1 as the paired noise term;
      - a known standardized shift with one of three predeclared geometries.
    ...
    Selection rule:
      1. empirical null FPR <= instrument_null_fpr_max;
      2. minimum power across all three geometries at target shift norm is
         >= instrument_target_power;
      3. among eligible instruments, maximize minimum power, then mean power.

    If none pass, the measurement remains unresolved and a negative real-data
    result must not be converted into absence of an arrangement effect.
    """
    ...
    # Store detection counts by instrument / effect family / strength.
    counts = {
        name: {
            family: {float(s): 0 for s in strengths}
    ...
    pvalue_sums = {
        name: {
            family: {float(s): 0.0 for s in strengths}
            for family in directions
        }
        for name in CANDIDATE_INSTRUMENTS
    }
    ...
    for rep in tqdm(range(reps), desc="Stage 5 instrument calibration"):
        base_idx = rng.integers(0, len(Xbase), size=n)
        noise_idx = rng.integers(0, len(noise_pool), size=n)

        A = Xbase[base_idx].copy()
        # Noise vector is B-A under an independently stochastic no-treatment fork.
        base_noise = noise_pool[noise_idx].copy()
        ...
        for family_i, (family, info) in enumerate(directions.items()):
            ...
            for strength_i, strength in enumerate(strengths):
                # Under strength=0, the arms differ only by empirically observed
                # stochastic no-treatment noise. Positive strengths add a known
                # treatment-direction shift in standardized feature space.
                B = A + base_noise
                if float(strength) != 0.0:
                    shifted = apply_standardized_shift(A, direction, float(strength))
                    B = B + (shifted - A)
                ...
                for inst_i, name in enumerate(CANDIDATE_INSTRUMENTS):
                    ...
                    counts[name][family][float(strength)] += int(p < 0.05)
                    pvalue_sums[name][family][float(strength)] += p
    ...
    for name in CANDIDATE_INSTRUMENTS:
        ...
        for family, info in directions.items():
            ...
            for strength in strengths:
                key = float(strength)
                rate = counts[name][family][key] / reps
                rows.append({
                    "shift_norm": key,
                    "detection_rate_alpha_0_05": rate,
                    "mean_p_value": pvalue_sums[name][family][key] / reps,
                })
                if abs(key - target_strength) < 1e-12:
                    target_powers.append(rate)
                if abs(key) < 1e-12:
        ...
        eligible = (
            worst_null_fpr <= fpr_max
            and worst_target_power >= target_power
        )
    ...
    result = {
        ...
        "selection_note": (
            "Instrument selection uses only synthetic calibration. Real matched-"
            "arrangement p-values from Stage 3 are not used to choose the winner."
        ),
    }

    reporter.json("stage-05-measurement-resolution.json", result)
    reporter.stage(
        "stage-05-measurement-resolution.md",
...
def stage_6_verdict(
    ...
    if selected is None:
        ...
        arrangement_statement = (
            "No candidate measurement instrument met the predeclared null-control "
            "and sensitivity requirements. The matched interior-timing hypothesis "
            "therefore remains unresolved under this quick protocol."
        )
    else:
        selected_real_test = (
        ...
        if selected_real_test["p_value"] < 0.05:
            ...
            arrangement_statement = (
                "A measurement instrument selected using synthetic calibration "
                "only detected a systematic matched interior-timing signature at "
                "the declared endpoint. This remains exploratory and requires an "
                "independent frozen confirmation before Chapter 18 can use it."
            )
        else:
            arrangement_status = "FAILED"
            arrangement_statement = (
                "The selected instrument met the predeclared calibration target "
                "but did not establish a systematic matched interior-timing "
                "signature at the declared endpoint under this protocol."
            )
    ...
    result = {
        ...
        "next_decision_logic": {
            "if_no_instrument_is_ready": (
                "Do not scale the real matched-arrangement experiment blindly. "
                "Improve sample size or measurement design using the synthetic "
                "calibration benchmark first."
            ),
            "if_instrument_ready_and_real_positive": (
                "Freeze the selected instrument, endpoint, codewords and analysis "
                "plan, then run an independent confirmatory Chapter 17 replication "
                "before using the result in Chapter 18."
            ),
            "if_instrument_ready_and_real_negative": (
                "Record matched interior timing as FAILED under the calibrated "
                "protocol; do not reinterpret the old L3 decoder result as temporal "
                "retention."
            ),
            "if_superposition_residual_large": (
                "Investigate why the ensemble additive response model fails before "
                "using information-processing language."
            ),
        },
    }
...
def main():
    ...
    parser.add_argument(
        "--report-dir",
        type=Path,
        default=Path("research/digital-life/ch17-perturbation-dynamics-v3"),
    )
    parser.add_argument(
        "--image-dir",
        type=Path,
    ...
    metadata = {
        ...
        "version_3_focus": (
            "Measurement resolution: compare paired and marginal instruments "
            "using synthetic known-null/known-effect calibration before judging "
            "matched temporal arrangement."
        ),
    ...
    assert calibration_vectors != None
